...
RSpec.describe SolidusFriendlyPromotions::PromotionHandler::Coupon, type: :model do
  ...
  context "integration for surfacing eligibility errors", :integration do
    let(:shirt) { create(:product, name: "Shirt", price: 10) }
    let(:cap) { create(:product, name: "Cap", price: 25 ) }
    let(:glass) { create(:product, name: "Glass", price: 64) }

    let!(:coupon_promotion) { coupon_promotion_code.promotion }
    let(:coupon_promotion_code) { create(:friendly_promotion_code, value: "10percentoff") }
    let!(:coupon_action) { SolidusFriendlyPromotions::Actions::AdjustLineItem.create(promotion: coupon_promotion, calculator: coupon_calculator) }
    let(:coupon_calculator) { SolidusFriendlyPromotions::Calculators::Percent.new(preferred_percent: 10) }

    let!(:shirt_promotion) { create(:friendly_promotion, name: "ShirtPromotion", apply_automatically: true) }
    let(:shirt_promotion_rule) { SolidusFriendlyPromotions::Rules::Product.new([shirt]) }
    let!(:shirt_action) { SolidusFriendlyPromotions::Actions::AdjustLineItem.create(promotion: shirt_promotion, calculator: shirt_calculator) }
    let(:shirt_calculator) { SolidusFriendlyPromotions::Calculators::FlatRate.new(preferred_amount: 1) }

    # future: must have at least 3 caps to be eligible
    # let!(:cap_promotion) { create(:friendly_promotion, name: "Caps10distributed", apply_automatically: true) }
    # let(:cap_promotion_rule) { SolidusFriendlyPromotions::Rules::Product.new([cap]) }
    # let!(:cap_action) { SolidusFriendlyPromotions::Actions::AdjustLineItem.create(promotion: cap_promotion, calculator: cap_calculator) }
    # let(:cap_calculator) { SolidusFriendlyPromotions::Calculators::DistributedAmount.new(preferred_amount: 10) }

    let(:order) { create(:order, coupon_code: "10percentoff") }

    # todo: add lanes too

    before do
      create(:line_item, product: shirt, order: order, price: shirt.price)
      create(:line_item, product: cap, order: order, quantity: 5, price: cap.price)
      create(:line_item, product: glass, order: order, price: glass.price)
      order.line_items.reload
      order.recalculate
    end

    it "successfully activates coupon promo" do
      # expect(order.total).to eq(10 + 25 * 5 + 64) # 196 not 199?
      subject.apply
      expect_order_connection(order: order, promotion: coupon_promotion, promotion_code: coupon_promotion_code)
      expect(subject.success).to be_present
      # order.line_items.each do |line_item|
      #   expect_adjustment_creation(adjustable: line_item, promotion: coupon_promotion, promotion_code: coupon_promotion_code)
      # end
      expect(order.reload.total).to eq(199 * 0.9)
    end

    it "successfully activates shirt promo" do
      subject.apply
      expect_order_connection(order: order, promotion: shirt_promotion)
      expect(subject.success).to be_present
      expect(order.reload.total).to eq(199 - 1)
    end
    ...
    context "with a whole-order adjustment action" do
      ...
      context "right coupon given" do
        ...
        context "when the coupon fails to activate" do
          before { allow_any_instance_of(SolidusFriendlyPromotions::PromotionHandler::Coupon).to receive(:apply).and_return false }
